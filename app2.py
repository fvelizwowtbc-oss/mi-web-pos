...
def guardar_imagen_producto(file, user_id, producto_id):
    ...
    try:
        ...
        return filename
    except Exception as e: logging.error(f"Error imagen: {e}"); return None
...
def obtener_imagen_producto(user_id, producto_id):
    try:
        ...
        return os.path.basename(glob.glob(os.path.join(user_dir, f"*user_{user_id}_prod_{producto_id}_*"))[0]) if glob.glob(os.path.join(user_dir, f"*user_{user_id}_prod_{producto_id}_*")) else None
    except: return None
...
def init_main_db():
    ...
    try:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS config_empresa (
            ...
            FOREIGN KEY(usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE)''')
        for col in ['suscripcion_activa','fecha_fin_suscripcion','limite_productos','limite_ventas_mensuales']:
            try: conn.execute(f'ALTER TABLE usuarios ADD COLUMN {col} {"BOOLEAN DEFAULT FALSE" if col=="suscripcion_activa" else "DATE" if col=="fecha_fin_suscripcion" else "INTEGER DEFAULT 50" if "productos" in col else "INTEGER DEFAULT 20"}')
            except: pass
...
def init_user_db(user_id):
    ...
    try:
        ...
        conn.execute('''CREATE TABLE IF NOT EXISTS clientes (
            ...
            email TEXT, direccion TEXT, documento TEXT,
            creado_en DATETIME DEFAULT CURRENT_TIMESTAMP)''')
        ...
        conn.commit(); logging.info(f"BD creada para usuario {user_id}")
...
def obtener_suscripcion_activa(uid):
    try:
        conn = sqlite3.connect(DATABASE_PRINCIPAL, timeout=20); conn.row_factory = sqlite3.Row
        s = conn.execute("SELECT s.*, u.nombre as un, u.email as ue FROM suscripciones s JOIN usuarios u ON s.usuario_id=u.id WHERE s.usuario_id=? AND s.estado='activa' AND s.fecha_fin>=date('now') ORDER BY s.fecha_fin DESC LIMIT 1",(uid,)).fetchone()
...
def cantidad_productos_usuario(uid):
    try:
        ...
        c = sqlite3.connect(p,timeout=20).execute('SELECT COUNT(*) as c FROM productos').fetchone()['c']; return c
    except: return 0
...
# ============ FUNCIONES DE CLIENTES Y FACTURAS POR COBRAR ============
...
def agregar_cliente(uid,datos):
    ...
    except Exception as e: logging.error(f"Error cliente: {e}"); return None
...
def crear_factura_cobrar(uid,venta_id,total_usd,total_ves,tipo_venta,cliente_id=None,cliente_nombre='Consumidor Final',nro_factura='',fecha_vencimiento=None):
    try:
        ...
        c.commit(); c.close(); return True
    except Exception as e: logging.error(f"Error factura cobrar: {e}"); return False
...
def obtener_facturas_cobrar(uid,filtro='todas'):
    try:
        ...
        if not os.path.exists(p): return []
        c = sqlite3.connect(p,timeout=20); c.row_factory = sqlite3.Row
        q = 'SELECT fc.*, v.creado_en as fecha_venta FROM facturas_cobrar fc LEFT JOIN ventas v ON fc.venta_id=v.id WHERE 1=1'
        ...
        for f in facturas:
            ...
            res.append(fd)
        c.close(); return res
    except Exception as e: logging.error(f"Error facturas: {e}"); return []
...
def agregar_abono(uid,fid,monto_usd,monto_ves,tasa_usada,metodo_pago='efectivo',notas=''):
    try:
        ...
        c.commit(); c.close(); return True
    except Exception as e: logging.error(f"Error abono: {e}"); return False
...
def actualizar_precio_factura_credito(uid,fid,nuevo_total_usd,nuevo_total_ves):
    try:
        ...
        c.commit(); c.close(); return True
    except Exception as e: logging.error(f"Error actualizando precio: {e}"); return False

# ============ FUNCION DE FACTURA ============
def generar_factura_venta(venta_id,productos_vendidos,tasas,uid,vendedor_nombre,metodo_pago,observaciones,tipo_venta='contado',cliente_nombre='Consumidor Final'):
    ...
    tipo_label = 'CONTADO' if tipo_venta=='contado' else 'CRÉDITO'
...
<div class="info"><div><strong>FACTURA:</strong> {nf}<br><strong>Fecha:</strong> {fs}<br><strong>Vendedor:</strong> {vendedor_nombre}</div><div><strong>Cliente:</strong> {cliente_nombre}<br><strong>Pago:</strong> {metodo_pago.upper()}<br><strong>Tasa:</strong> {tasas.get('activa','Oficial').upper()}</div></div>
...
<div class="footer"><div class="mensaje">{config.get('mensaje_factura','Gracias por su compra!')}</div><p>Generado el {fs}</p></div>
...
def procesar_venta():
    ...
    try:
        ...
        fp,fn,nf=generar_factura_venta(vid,vp,tasas,uid,vn,data.get('metodo_pago','Efectivo'),data.get('observaciones',''),tipo_venta,cliente_nombre)
        conn.execute('UPDATE ventas SET nro_factura=? WHERE id=?',(nf,vid))
        # Crear factura por cobrar si es crédito
...
def user_reportes():
    ...
    try:
        dbp=os.path.join(app.config['DATABASE_DIR'],f'user_{uid}.db')
        conn=sqlite3.connect(dbp,timeout=20); conn.row_factory=sqlite3.Row
        vpm=conn.execute("SELECT strftime('%Y-%m',creado_en) as mes, COUNT(*) as c, SUM(total_usd) as tu, SUM(total_oficial) as tv FROM ventas GROUP BY mes ORDER BY mes DESC LIMIT 12").fetchall()
        tp=conn.execute('SELECT p.nombre, p.codigo, SUM(v.cantidad) as tv, SUM(v.total_usd) as i FROM ventas v JOIN productos p ON v.producto_id=p.id GROUP BY p.id ORDER BY tv DESC LIMIT 10').fetchall()
        ...
        stats=conn.execute('SELECT COUNT(*) as tp, SUM(stock_actual) as st, (SELECT COUNT(*) FROM ventas) as tv, (SELECT COALESCE(SUM(total_usd),0) FROM ventas) as it FROM productos').fetchone()
        conn.close()
...
def gestionar_clientes():
    ...
    if request.method=='POST':
        ...
        if not datos['nombre']: flash('Nombre requerido','danger')
            ...
            agregar_cliente(uid,datos); flash('Cliente agregado','success')
...
@app.route('/user/facturas_cobrar')
@login_required
def ver_facturas_cobrar():
    uid=session['user_id']; init_user_db(uid)
    filtro=request.args.get('filtro','todas')
    facturas=obtener_facturas_cobrar(uid,filtro)
    return render_template('user/facturas_cobrar.html',facturas=facturas,filtro=filtro)
...
def abonar_factura(factura_id):
    ...
    try:
        ...
        if monto_usd<=0: flash('Monto requerido','danger')
            ...
            agregar_abono(uid,factura_id,monto_usd,monto_ves,tasa_usada,metodo,notas)
            flash('Abono registrado','success')
    except: flash('Error','danger')
    return redirect(url_for('ver_facturas_cobrar'))
...
@login_required
def listar_facturas():
    uid=session['user_id']; fd='facturas'; facturas=[]
    if os.path.exists(fd):
        for f in sorted(os.listdir(fd),reverse=True):
            if f.endswith('.html') and f'_{uid}.html' in f:
                partes=f.replace('factura_','').replace('.html','').split('-')
                facturas.append({'filename':f,'url':f'/ver_factura/{f}','nro':f"F-{partes[0]}" if partes else f,'fecha':f"{partes[0][:4]}-{partes[0][4:6]}-{partes[0][6:8]}" if len(partes[0])>=14 else 'N/A'})
...
def admin_suscripciones():
    try:
        conn=sqlite3.connect(DATABASE_PRINCIPAL,timeout=20); conn.row_factory=sqlite3.Row
        sus=conn.execute('SELECT s.*, u.nombre as un, u.email as ue FROM suscripciones s JOIN usuarios u ON s.usuario_id=u.id ORDER BY s.creado_en DESC').fetchall()
...
def actualizar_tasa_oficial():
    ...
    except: return jsonify({'success':False,'error':str(e)})
...
@app.route('/api/obtener_cliente/<int:cliente_id>')
@login_required
def api_obtener_cliente(cliente_id):
    ...
    try:
        ...
        conn.close()
        return jsonify({'success':True,'cliente':dict(c) if c else None})
    except: return jsonify({'success':False})
