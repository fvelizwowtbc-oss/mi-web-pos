...
def agregar_abono(uid, fid, monto_usd, monto_ves, tasa_usada, metodo_pago='efectivo', notas=''):
    try:
        p = os.path.join(app.config['DATABASE_DIR'], f'user_{uid}.db')
        conn = sqlite3.connect(p, timeout=30); conn.execute('PRAGMA busy_timeout = 5000')
        ...
        fc = conn.execute('SELECT saldo_usd,saldo_ves FROM facturas_cobrar WHERE id=?', (fid,)).fetchone()
        if fc:
            nu = max(0, fc['saldo_usd'] - monto_usd); nv = max(0, fc['saldo_ves'] - monto_ves)
            ne = 'pagado' if nu <= 0 else 'pendiente'
            conn.execute('UPDATE facturas_cobrar SET saldo_usd=?,saldo_ves=?,estado=? WHERE id=?', (nu, nv, ne, fid))
...
def procesar_venta():
    ...
    try:
        ...
        for item in data['items']:
            ...
            if not p: conn.close(); return jsonify({'success':False,'error':'Producto no encontrado'})
        ...
        # Insertar factura_cobrar en la MISMA conexión
...
def listar_facturas():
    ...
    for fc in facturas_bd:
        ...
        facturas.append({
            ...
            'cliente': cliente_info, 'total_usd': fc.get('total_usd', 0),
            'total_ves': fc.get('total_ves', 0), 'saldo_usd': fc.get('saldo_usd', 0),
            'saldo_ves': fc.get('saldo_ves', 0), 'estado': fc.get('estado', 'pagado'),
            'tipo_venta': fc.get('tipo_venta', 'contado'),
            'abonos': fc.get('abonos', []),
            'total_abonado_usd': fc.get('total_abonado_usd', 0),
            'filename': f"factura_{nro.replace('/', '-')}_{uid}.html" if nro else '',
            'url': f"/ver_factura/factura_{nro.replace('/', '-')}_{uid}.html" if nro else '',
            'cliente_doc': fc.get('cliente_doc', ''), 'cliente_tel': fc.get('cliente_tel', '')
        })
    ...
    if os.path.exists(facturas_dir):
        for f in sorted(os.listdir(facturas_dir), reverse=True):
            if f.endswith('.html') and f'_{uid}.html' in f:
                ...
                if nro not in nros_vistos:
                    facturas.append({
                        'id': None, 'nro_factura': nro, 'fecha': '',
                        'cliente': 'Consumidor Final', 'total_usd': 0, 'total_ves': 0,
                        'saldo_usd': 0, 'saldo_ves': 0, 'estado': 'generada',
                        'tipo_venta': 'contado', 'abonos': [], 'total_abonado_usd': 0,
                        'filename': f, 'url': f'/ver_factura/{f}'
                    })
...
def abonar_factura(factura_id):
    ...
    try:
        ...
        if monto_usd <= 0: flash('Monto requerido', 'danger')
            ...
            if ok: flash('Pago registrado correctamente', 'success')
            else: flash('Error al registrar pago', 'danger')
    except: flash('Error', 'danger')
