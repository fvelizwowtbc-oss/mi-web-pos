...
def agregar_abono(uid, fid, monto_usd, monto_ves, tasa_usada, metodo_pago='efectivo', notas=''):
    try:
        ...
        # Insertar abono
        conn.execute('''INSERT INTO abonos (factura_id, monto_usd, monto_ves, tasa_usada, metodo_pago, notas)
            VALUES (?,?,?,?,?,?)''',
            (fid, round(monto_usd, 2), round(monto_ves, 2), round(tasa_usada, 2), metodo_pago, notas))
        
        # Obtener datos actuales
        fc = conn.execute('SELECT total_usd, total_ves, saldo_usd, saldo_ves FROM facturas_cobrar WHERE id=?', (fid,)).fetchone()
        ...
        if fc:
            ...
            # Si el saldo es menor a 0.01, considerarlo como pagado
            nuevo_estado = 'pagado' if nuevo_saldo_usd < 0.01 else 'pendiente'
            
            # Actualizar
            conn.execute('''UPDATE facturas_cobrar 
                SET saldo_usd = ?, saldo_ves = ?, estado = ?
                WHERE id = ?''',
                (nuevo_saldo_usd, nuevo_saldo_ves, nuevo_estado, fid))
            
            conn.commit()
            
            ver = conn.execute('SELECT saldo_usd, estado FROM facturas_cobrar WHERE id=?', (fid,)).fetchone()
            logging.info(f"[ABONO] Factura#{fid}: Monto={monto}, Saldo={ver['saldo_usd']}, Estado={ver['estado']}")
...
def user_dashboard():
    ...
    try:
        ...
        # Stats con conteo correcto de stock bajo
        stats = conn.execute('''
            SELECT 
                ...
                COALESCE((SELECT SUM(total_usd) FROM ventas WHERE strftime("%Y-%m", creado_en) = strftime("%Y-%m", "now")), 0) as imu
        ''').fetchone()
    ...
    try:
        ...
        ventas_por_mes = conn.execute("""
            SELECT strftime('%Y-%m', creado_en) as mes, 
                   ...
                   COALESCE(SUM(total_oficial), 0) as total_oficial
            FROM ventas 
            GROUP BY mes 
            ORDER BY mes DESC 
            LIMIT 12
        ...
        top_productos = conn.execute("""
            SELECT p.nombre, 
                   ...
                   COALESCE(SUM(v.total_usd), 0) as ingresos
            FROM ventas v 
        ...
        stats_row = conn.execute("""
            SELECT 
                COALESCE((SELECT COUNT(*) FROM productos), 0) as total_productos,
                COALESCE((SELECT SUM(stock_actual) FROM productos), 0) as stock_total,
                COALESCE((SELECT COUNT(*) FROM ventas), 0) as total_ventas,
                COALESCE((SELECT SUM(total_usd) FROM ventas), 0) as ingresos_totales
        """).fetchone()
        
        if stats_row:
            stats = {
                'total_productos': stats_row['total_productos'] or 0,
                'stock_total': stats_row['stock_total'] or 0,
                'total_ventas': stats_row['total_ventas'] or 0,
                'ingresos_totales': stats_row['ingresos_totales'] or 0
            }
        ...
        flash('Error cargando reportes', 'danger')
...
def editar_cliente(cliente_id):
    ...
    try:
        ...
        flash('Error al editar cliente', 'danger')
...
def listar_facturas():
    ...
    # SOLO obtener facturas de la BD (no HTML antiguas)
    ...
    for fc in facturas_bd:
        ...
        facturas.append({
            ...
            'total_usd': round(fc.get('total_usd', 0), 2),
            'total_ves': round(fc.get('total_ves', 0), 2),
            'saldo_usd': round(fc.get('saldo_usd', 0), 2),
            'saldo_ves': round(fc.get('saldo_ves', 0), 2),
            'estado': fc.get('estado', 'pagado'),
            'tipo_venta': fc.get('tipo_venta', 'contado'),
            'abonos': fc.get('abonos', []),
            'total_abonado_usd': round(fc.get('total_abonado_usd', 0), 2),
